...
def _dedupe_sorted(df):
    ...
    # utc=True normalizes tz-naive/tz-aware timestamps to a common, comparable
    # basis - rows can otherwise mix representations (e.g. a freshly fetched
    # tz-aware yfinance row alongside a differently-parsed on-disk row).
    ...
    order = pd.to_datetime(df.index, utc=True).argsort(kind='stable')
...
def write_incremental(folder, ticker, new_rows):
    """
    ...
    Always re-materializes the legacy flat file. Returns a small status dict.
    """
    ...
    materialize_legacy(folder, ticker)
    return {'rolled_rows': len(rolls), 'current_rows': len(stays)}


def rebuild_archive_current(folder, ticker, full_data):
    """
    ...
    # full_data (already deduped/sorted) IS archive+current combined - stays/rolls
    # is just a filter over it, so reuse it instead of reading both tiers back.
    materialize_legacy(folder, ticker, combined=full_data)
...
def check_and_handle_split(folder, ticker, interval, split_rows, fetch_fn,
    ...
    result = rebuild_archive_current(folder, ticker, full_data)
